...
public class HandleExtraFileTypes extends ImagePlus implements PlugIn {
    ...
    private Object tryOpen(String directory, String name, String path) {
        ...
        // Temporarily suppress "plugin not found" errors if LOCI Bio-Formats plugin is
        // installed
        ...
        // IXI Custom Reader Addition
        // Must be checked BEFORE the standard Analyze/NIfTI handler (which also catches
        // .img)
        if (name.endsWith(".img") || name.endsWith(".prj")) {
            try {
                // Use reflection to call the 'open' method which returns ImagePlus
                // This allows falling back to standard readers if this one fails.
        ...
        // Original logic for .img files (InstantImager and Analyze/NIfTI)
        if (name.endsWith(".img") && buf[0] == 75 && buf[1] == 65 && buf[2] == 74 && buf[3] == 0) {
            return tryPlugIn("InstantImager_Reader", path);
        }
        ...
        // Analyze format (.img/.hdr) handler
        // Opens the file using the Nifti_Reader if it is installed,
        // otherwise the Analyze_Reader is used.
        if (name.endsWith(".img") || name.endsWith(".hdr")) {
            if (Menus.getCommands().get("NIfTI-Analyze") != null)
                return tryPlugIn("Nifti_Reader", path);
            else
